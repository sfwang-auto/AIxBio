...
export default async function ArticlePage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <main className="article-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, '\\u003c') }} />
    <header className="article-hero"><Link className="back-link" href={`/${locale}/articles`}><ArrowLeft /> {copy.backArticles}</Link><div className="article-kicker">{article.topics.map((topic) => <Link key={topic} href={`/${locale}/topics/${topic}`}>{topicName(topic, locale)}</Link>)}</div><h1>{text.title}</h1><div className="article-byline"><span><Clock3 /> {text.readingMinutes} {copy.minutes}</span></div></header>
