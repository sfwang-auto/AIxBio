...
export default async function LocalizedHome({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main>
    ...
    <section className="home-section"><div className="section-heading"><div><h2>{copy.latestEssays}</h2></div><Link href={`/${locale}/articles`}>{copy.allEssays} <ArrowRight /></Link></div><div className="article-list">{articles.slice(0, 2).map((article, index) => <ArticleCard key={article.slug} article={article} locale={locale} index={index} />)}</div></section>
  </main>;
}
