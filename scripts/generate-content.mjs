...
function runSelfTest() {
  ...
  const validArticle = { slug: 'test-article', topics: ['test'], draft: false, translations: { zh: { title: '测试' }, en: { title: 'Test' } } };
  const cases = [
...
async function main() {
  ...
  console.log(`Validated ${allArticleData.length} bilingual articles (${articleData.length} published, ${allArticleData.length - articleData.length} draft) and ${topicData.length} topics.`);
}
