...
import { authorsShort, formatDate, topicName } from '@/lib/content';
...
export function ArticleCard({ article, locale, index }: { article: ArticleRecord; locale: Locale; index?: number }) {
  ...
  return (
    <article className="article-card">
      ...
      <div>
        <div className="card-meta"><time dateTime={article.date}>{formatDate(article.date, locale)}</time><span>{text.readingMinutes} {copy.minutes}</span>{article.demo && <span className="demo-label">{copy.demo}</span>}</div>
...
export function PaperCard({ paper, locale, index: _index }: { paper: PaperRecord; locale: Locale; index?: number }) {
  ...
  return (
    <article className="paper-card">
      <div className="paper-card-top"><span>{paper.venue} · {paper.year}</span>{paper.demo && <span className="demo-label">{copy.demo}</span>}</div>
